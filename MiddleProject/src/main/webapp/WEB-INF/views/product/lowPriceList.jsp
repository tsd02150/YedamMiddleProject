...
<div style="display: block;">
	...
	<c:choose>
		...
		<div class="untree_co-section product-section before-footer-section"
			...
			<table>
				<tr>
				<c:forEach var="board" items="${lowPriceList }">
			...
			<div class="pagination">
				<c:if test="${pageInfo.prev}">
					<a href="lowPriceList.do?page=${pageInfo.startPage-1 }">Prev</a>
				</c:if>
				<c:forEach var="i" begin="${pageInfo.startPage }"
					end="${pageInfo.endPage }">
					<a href="lowPriceList.do?page=${i }"
						class=${i==pageInfo.pageNum?'active':'' }>${i }</a>
				</c:forEach>
				<c:if test="${pageInfo.next}">
					<a href="lowPriceList.do?page=${pageInfo.endPage+1 }">Next</a>
				</c:if>
			</div>
		</div>
	</div>
